                    ...
                    task::Poll::Pending => match self.receiver.try_next() {
                        Ok(Some(control)) => match control {
                            ...
                            Control::CreateWindow {
                                ...
                                // VENDORED ADDITION: enable IME so dead-key accents
                                // compose in the terminal (¨+e → ë). macOS only —
                                // Windows/Linux already deliver composed text via
                                // `KeyEvent.text`, and enabling IME there would route
                                // text through `Ime::Commit` and risk regressions.
                                #[cfg(target_os = "macos")]
