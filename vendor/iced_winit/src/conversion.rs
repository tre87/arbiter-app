...
pub fn window_event(
    ...
    match event {
        ...
        WindowEvent::KeyboardInput { event, .. } => Some(Event::Keyboard({
            ...
            // VENDORED note (macOS): we deliberately do NOT strip `text` here. With
            // IME enabled, ordinary keypresses (letters, Option-symbols @ { } [ ]) are
            // still delivered through `text` and do NOT fire `Ime::Commit`; only an
            // actual dead-key composition (¨+e) emits a commit — and winit reports
            // the composing keystroke with `text == None`, so there's no double.
        ...
        // VENDORED ADDITION (macOS): an IME commit — the result of dead-key
        // composition (¨+e → "ë") or any composed text — becomes a synthetic key
        // press carrying the text, so the app's existing text path types it.
        // Preedit/Enabled/Disabled are ignored (no inline preedit rendering yet).
        #[cfg(target_os = "macos")]
        WindowEvent::Ime(winit::event::Ime::Commit(text)) => {
            Some(Event::Keyboard(keyboard::Event::KeyPressed {
                key: keyboard::Key::Character(crate::core::SmolStr::new(&text)),
                modified_key: keyboard::Key::Character(crate::core::SmolStr::new(&text)),
                physical_key: keyboard::key::Physical::Unidentified(
                    keyboard::key::NativeCode::Unidentified,
                ),
                modifiers: keyboard::Modifiers::default(),
                location: keyboard::Location::Standard,
                text: Some(crate::core::SmolStr::new(&text)),
            }))
        }
