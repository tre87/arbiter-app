        ...
        let swap_chain = match self.swap_chain.write().take() {
            ...
            None => {
                let desc = d3d12::SwapchainDesc {
                    ...
                    // Arbiter: present an HWND swapchain 1:1. With Stretch, DWM scales the
                    // frame whenever the client rect and the visible area differ, which
                    // winit's undecorated_shadow makes them do by one row, and the text
                    // goes soft. Composition swapchains accept only Stretch.
