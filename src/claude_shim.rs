...
/// `arbiter` in a pane: a launcher that runs THIS binary's `about` subcommand (see
/// `about.rs`), so the mark and build facts are one word away in every terminal. Best
/// effort: without a known path to ourselves there is simply no such command.
fn write_about_launchers(bin_dir: &Path) {
    let Ok(exe) = std::env::current_exe() else { return };
    let exe = exe.display().to_string();
    let posix = format!(
        "#!/bin/sh\n# Generated by Arbiter: `arbiter` in a pane shows the app's mark and build facts.\nexec \"{}\" about \"$@\"\n",
        exe.replace('\\', "/")
    );
    let posix_path = bin_dir.join("arbiter");
    if std::fs::write(&posix_path, posix).is_ok() {
        make_executable(&posix_path);
    }
    #[cfg(windows)]
    {
        let cmd = format!(
            "@echo off\r\nrem Generated by Arbiter: `arbiter` in a pane shows the app's mark and build facts.\r\n\"{exe}\" about %*\r\n"
        );
        let _ = std::fs::write(bin_dir.join("arbiter.cmd"), cmd);
    }
}
