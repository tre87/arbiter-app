...
fn overview_titlebar(state: &State) -> Element<'_, Message> {
    ...
    // Windows has caption buttons on the right, so a centred title collides with them —
    // LEFT-align it there (brand, draggable fill, caption buttons flush right). macOS
    // has no right-side caption (traffic lights are on the left), so CENTER the brand
    // over the full width. The brand + the empty area both drag the window.
    #[cfg(any(target_os = "windows", target_os = "macos"))]
